...
type daemon struct {
	...
	// lastRoutingByEngine caches the per-engine declarative routing metadata
	// (capabilities, aliases, capacity, priority, timeouts) from the same
	// /v1/models fetch, so a transient miss reuses it rather than blanking a
	// remote node's routing state. Keyed by hostUuid.
	lastRoutingByEngine map[string]map[string]*noderec.EngineRouting
...
func (d *daemon) dropSelf(oldHostUUID, newHostUUID string) {
	...
	if routing, ok := d.lastRoutingByEngine[oldHostUUID]; ok {
		d.lastRoutingByEngine[newHostUUID] = routing
	}
...
func (d *daemon) fetchModels(ip string, port int, clusterUUID string) ([]string, map[string][]string, map[string][]string, map[string]*noderec.EngineRouting, bool) {
	...
	var body struct {
		...
		RoutingByEngine map[string]*noderec.EngineRouting `json:"routingByEngine"`
	}
